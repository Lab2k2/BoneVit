...
class MedViT(nn.Module):
    def __init__(self, stem_chs, depths, path_dropout, attn_drop=0, drop=0, num_classes=1000,
        ...
        self.stage_block_types = [[DenseBlock] * depths[0],  # Thay thế ECB bằng DenseBlock
                                  [DenseBlock] * (depths[1] - 1) + [LTB],
                                  [DenseBlock, DenseBlock, DenseBlock, DenseBlock, LTB] * (depths[2] // 5),
                                  [DenseBlock] * (depths[3] - 1) + [LTB]]
        ...
        for stage_id in range(len(depths)):
            ...
            for block_id in range(numrepeat):
                ...
                if block_type is DenseBlock:
                    layer = DenseBlock(num_layers=4, in_channels=input_channel, growth_rate=output_channel // 4)
    ...
    def merge_bn(self):
        ...
        for idx, module in self.named_modules():
            if isinstance(module, DenseBlock) or isinstance(module, LTB):
                module.merge_bn()
